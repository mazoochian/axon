...
See [ROADMAP.md](ROADMAP.md) for the phase history (Phase 8 through 27 and counting — the roadmap as originally scoped is complete; ongoing phases are new features plus a compliance/hardening pass against the [Complement](#compliance-testing) acceptance suite).
...
As of Phase 27 (see [ROADMAP.md](ROADMAP.md) for the full trail):

- **`tests/csapi` (core Client-Server API): 63/64**. Phase 24 closed the point-in-time-state gap Phase 22 had left open (`TestGetRoomMembersAtPoint` and `TestLeftRoomFixture`, confirmed individually via a clean before/after Complement diff) — that commit's own feature had real edges it didn't cover; see the Phase 24 entry for what was actually missing. The one remaining failure is a scenario even Synapse doesn't reliably pass (a documented upstream Synapse issue Complement skips it for). A full whole-package re-run wasn't completed this phase — see the Phase 24 entry. Separately, Phase 26 individually re-ran six longer-standing `csapi` names Phase 20 had left with hedged language (`TestAsyncUpload`, `TestE2EKeyBackupReplaceRoomKeyRules`, `TestServerNotices`, `TestSearch`, `TestDeviceListUpdates`, `TestPresence`) — all six pass, confirming Phase 20's own final 61/64 tally was right and closing out the lingering "not fully triaged"/"not root-caused" phrasing that outlived the fixes.
- **`tests/` (top-level federation/room package): 61/88 (~69%) as of Phase 22; the auth-chain/rejection cluster is now individually confirmed (Phase 25).** The dominant blocker is a `send_join` signature-verification mismatch — confirmed, across **four** independent from-scratch cryptographic verifications (Phase 25 re-confirmed it a fourth time rather than trusting the prior three at face value; Phase 26's run reproduced the identical symptom a fifth time, on `TestCorruptedAuthChain`, without re-opening the crypto investigation itself), to **not** be an axon bug (most likely a bug in a dev-snapshot dependency Complement's own harness pins). It alone gates the entire knocking feature family and most of the restricted-rooms cluster (~13 tests), and — specifically — `TestCorruptedAuthChain`, whose own auth-chain-corruption assertions never run at all because the test fails earlier, at its federated `send_join`. The `soft_failed` gap Phase 22 flagged as a plausible explanation for "a chunk of the remaining auth-chain/rejection-handling failures" is now closed (Phase 25: `TestInboundFederationRejectsEventsWithRejectedAuthEvents` and `TestUnrejectRejectedEvents` both confirmed passing). Phase 27 flipped four more named tests in this package clean (`TestNetworkPartitionOrdering`, `TestInboundCanReturnMissingEvents`, `TestDeviceListsUpdateOverFederation`, `TestToDeviceMessagesOverFederation` — see below and the Phase 27 entry), each confirmed individually rather than via a fresh whole-package run, so the `61/88` figure itself is stale on the low side but not re-tallied here.
- `tests/msc*` (unstable MSCs): still expected failures for the general pool — axon has never claimed to implement unstable MSCs — but two specific names worth naming directly, since Phase 26 traced them past a resemblance into a confirmed cause: `TestMSC4297StateResolutionV2_1_starts_from_empty_set` and `..._includes_conflicted_subgraph` both fail inside Complement's own pinned `gomatrixserverlib` snapshot (`eventauth: invalid ID` on a v12 room ID) before axon's state-res code ever runs — the same dev-dependency error class Phase 22 only guessed at for a different v12 test.

**Four of Phase 26's residuals are now closed, each confirmed individually against a freshly rebuilt image (Phase 27):**

- `TestNetworkPartitionOrdering` — **now passes.** Phase 23's depth-first `dir=b` ordering fix was real but only narrowed the failure; the actual remaining bug was a `prev_batch` off-by-one in `SyncController` one layer above the ordering itself. See the Phase 27 entry.
- `TestInboundCanReturnMissingEvents` — **now passes, all 4 subtests.** Not the redaction gap it looked like: three distinct bugs in `get_missing_events`' own walk/visibility/dedup handling. See the Phase 27 entry.
- `TestDeviceListsUpdateOverFederation` / `TestToDeviceMessagesOverFederation` — **both now pass, all subtests.** The "device-list/to-device over federation timeouts" bullet (open since Phase 17) is closed: four compounding bugs in remote-invite routing, join detection, fanout-cursor persistence across restarts, and outbound-queue backoff. See the Phase 27 entry.
- `TestJumpToDateEndpoint` — **still fails overall**, but meaningfully narrowed: the local-history-distrust heuristic fix now correctly handles both explicitly-in-scope federation subtests ("looking forwards"/"looking backwards"), leaving 2 remaining failures on a separate, newly-identified root cause (`GET /context`'s pagination boundary assumes `stream_ordering` tracks depth, which a federation-backfilled event breaks). See the Phase 27 entry.
