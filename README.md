...
See [ROADMAP.md](ROADMAP.md) for the phase history (Phase 8 through 26 and counting — the roadmap as originally scoped is complete; ongoing phases are new features plus a compliance/hardening pass against the [Complement](#compliance-testing) acceptance suite).
...
As of Phase 26 (see [ROADMAP.md](ROADMAP.md) for the full trail):
...
**Re-verified, and only partly closed**: Phase 23 (`mix test`-only, no live Complement run at the time) landed fixes targeting `TestInboundCanReturnMissingEvents`, `TestNetworkPartitionOrdering`, and `TestJumpToDateEndpoint`. Phase 26 ran all three individually against live Complement: **all three still fail.** Each fix was real and independently correct on its own terms, but none was the test's only blocker — `TestNetworkPartitionOrdering` now fails at a narrower point in its own pagination ordering, `TestInboundCanReturnMissingEvents` fails on an entirely different symptom (a `get_missing_events` response missing `msgtype`, not the event-sequence shift the guest_access fix targeted), and `TestJumpToDateEndpoint`'s federation subtests are still returning the wrong event id. See the Phase 26 entry in [ROADMAP.md](ROADMAP.md) for what each one now shows. Also newly scoped this phase: the "device-list/to-device over federation timeouts" bullet (open since Phase 17, never narrowed) — `TestDeviceListsUpdateOverFederation` and `TestToDeviceMessagesOverFederation` still time out, but specifically on the *initial* invite/to-device delivery path; the room-join and device-list-visibility variants of the same feature both pass.
