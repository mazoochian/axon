...
- **`tests/` (top-level federation/room package): 61/88 (~69%) as of Phase 22; the auth-chain/rejection cluster is now individually confirmed (Phase 25).** The dominant blocker is a `send_join` signature-verification mismatch — confirmed, across **four** independent from-scratch cryptographic verifications (Phase 25 re-confirmed it a fourth time rather than trusting the prior three at face value), to **not** be an axon bug (most likely a bug in a dev-snapshot dependency Complement's own harness pins). It alone gates the entire knocking feature family and most of the restricted-rooms cluster (~13 tests), and — specifically — `TestCorruptedAuthChain`, whose own auth-chain-corruption assertions never run at all because the test fails earlier, at its federated `send_join`. The `soft_failed` gap Phase 22 flagged as a plausible explanation for "a chunk of the remaining auth-chain/rejection-handling failures" is now closed (Phase 25: `TestInboundFederationRejectsEventsWithRejectedAuthEvents` and `TestUnrejectRejectedEvents` both confirmed passing).
- `tests/msc*` (unstable MSCs) not run — expected failures, axon has never claimed to implement unstable MSCs.

**Not yet re-verified**: Phase 23 (`mix test`-only, no live Complement run) landed fixes targeting `TestInboundCanReturnMissingEvents`, `TestNetworkPartitionOrdering`, and `TestJumpToDateEndpoint` — see the Phase 23 entry in [ROADMAP.md](ROADMAP.md) for what each fix does and why it should move the `tests/` count above 61/88. Treat that number as stale until a fresh before/after Complement diff confirms the actual delta.
