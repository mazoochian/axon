defmodule AxonFederation.HttpClientTest do
  ...
  test "get/2 sends a well-formed X-Matrix authorization header" do
    FakeRemoteMatrixServer.put_response(@port, {"GET", "/_matrix/federation/v1/version"}, 200, %{
      "server" => %{"name" => "axon"}
    })

    assert {:ok, %{"server" => %{"name" => "axon"}}} =
             HttpClient.get(@server_name, "/_matrix/federation/v1/version")

    [%{headers: headers}] = FakeRemoteMatrixServer.requests(@port)
    {_, auth} = Enum.find(headers, fn {k, _} -> k == "authorization" end)

    assert auth =~
             ~r/^X-Matrix origin="[^"]+",destination="#{@server_name}",key="[^"]+",sig="[^"]+"$/
  end
  ...
  test "put/3 signs the body as part of the request and delivers it" do
    ...
    assert {:ok, %{}} =
             HttpClient.put(@server_name, "/_matrix/federation/v1/send/txn1", %{"pdus" => []})
  ...
  test "post/3 works the same as put for signing/delivery" do
    FakeRemoteMatrixServer.put_response(
      @port,
      {"POST", "/_matrix/federation/v1/get_missing_events/!room:x"},
      200,
      %{"events" => []}
    )

    assert {:ok, %{"events" => []}} =
             HttpClient.post(@server_name, "/_matrix/federation/v1/get_missing_events/!room:x", %{
               "limit" => 10
             })
  end

  test "a non-2xx response with a Matrix error body surfaces {errcode, error}" do
    FakeRemoteMatrixServer.put_response(@port, {"GET", "/_matrix/federation/v1/notfound"}, 404, %{
      "errcode" => "M_NOT_FOUND",
      "error" => "no such thing"
    })

    assert HttpClient.get(@server_name, "/_matrix/federation/v1/notfound") ==
             {:error, {"M_NOT_FOUND", "no such thing"}}
  end

  test "a non-2xx response without a Matrix error body surfaces the raw status" do
    FakeRemoteMatrixServer.put_response(@port, {"GET", "/_matrix/federation/v1/broken"}, 500, %{
      "unexpected" => "shape"
    })

    assert HttpClient.get(@server_name, "/_matrix/federation/v1/broken") ==
