defmodule AxonFederation.RoomKnockTest do
  ...
  test "happy path: knocks on a remote room and records the local knock + preview state", %{user_id: user_id} do
    room_id = remote_room_id()

    FakeRemoteMatrixServer.make_knock_response(@port, room_id, user_id, "10")

    preview = [%{"type" => "m.room.name", "state_key" => "", "sender" => "@owner:#{@server_name}", "content" => %{"name" => "Cool Room"}}]
    FakeRemoteMatrixServer.send_knock_response(@port, preview)

    assert RoomKnock.knock_via_federation(room_id, user_id, [@server_name], "let me in") == {:ok, room_id}
  ...
  test "the reason field round-trips into the signed knock event's content", %{user_id: user_id} do
    ...
    {:ok, ^room_id} = RoomKnock.knock_via_federation(room_id, user_id, [@server_name], "please let me in")

    [%{body: sent_knock_event}] =
      FakeRemoteMatrixServer.requests(@port) |> Enum.filter(&String.contains?(&1.path, "send_knock"))
  ...
  test "falls back to the next server in via_servers when the first fails", %{user_id: user_id} do
    ...
    assert RoomKnock.knock_via_federation(room_id, user_id, [bad_server, @server_name], nil) == {:ok, room_id}
  end

  test "returns :all_servers_failed when every server fails", %{user_id: user_id} do
    room_id = remote_room_id()
    assert RoomKnock.knock_via_federation(room_id, user_id, [@server_name], nil) == {:error, :all_servers_failed}
  end

  test "a malformed make_knock response is a failure, not a crash", %{user_id: user_id} do
    room_id = remote_room_id()
    FakeRemoteMatrixServer.put_response(@port, {"GET", ~r{^/_matrix/federation/v1/make_knock/}}, 200, %{"nope" => true})
