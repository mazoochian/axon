defmodule AxonMedia.Store do
  ...
  @id_bytes 18  # 18 bytes → 24 base64url chars
  ...
  def download(media_id) do
    case Repo.one(
      from m in "media",
        ...
        select: %{content_type: m.content_type, storage_path: m.storage_path}
    ) do
      nil -> {:error, :not_found}
      %{storage_path: nil} -> {:error, :not_found}
  ...
  @doc "Look up content-type for a media_id without reading the file."
  def get_meta(media_id) do
    Repo.one(
      from m in "media",
        where: m.media_id == ^media_id,
        select: %{content_type: m.content_type, origin_server: m.origin_server, storage_path: m.storage_path}
    )
  end
end
