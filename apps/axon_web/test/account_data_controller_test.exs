defmodule AxonWeb.AccountDataControllerTest do
  ...
  test "global account data put then get round-trips" do
    ...
    put_conn =
      authed(alice.token)
      |> jpu("/_matrix/client/v3/user/#{alice.user_id}/account_data/m.custom", %{"foo" => "bar"})

    assert put_conn.status == 200

    get_conn =
      authed(alice.token) |> get("/_matrix/client/v3/user/#{alice.user_id}/account_data/m.custom")
  ...
  test "getting account data that was never set is not_found" do
    ...
    conn =
      authed(alice.token)
      |> get("/_matrix/client/v3/user/#{alice.user_id}/account_data/m.never_set")
  ...
  test "cannot read another user's global account data" do
    ...
    authed(alice.token)
    |> jpu("/_matrix/client/v3/user/#{alice.user_id}/account_data/m.custom", %{"secret" => true})

    conn =
      authed(bob.token) |> get("/_matrix/client/v3/user/#{alice.user_id}/account_data/m.custom")
  ...
  test "cannot write another user's global account data" do
    ...
    conn =
      authed(bob.token)
      |> jpu("/_matrix/client/v3/user/#{alice.user_id}/account_data/m.custom", %{})
  ...
  test "room account data that was never set is not_found" do
    ...
    conn =
      authed(alice.token)
      |> get("/_matrix/client/v3/user/#{alice.user_id}/rooms/#{room_id}/account_data/m.never_set")
