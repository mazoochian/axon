defmodule AxonWeb.E2E.ModerationReportingFlowTest do
  ...
  test "report -> kick -> rejoin -> ban -> rejoin rejected -> unban -> rejoin" do
    ...
    offending_event_id =
      send_event(charlie.token, room_id, "m.room.message", %{"body" => "spammy nonsense"})

    # --- report ---
    report_conn =
      authed(alice.token)
      |> jp("/_matrix/client/v3/rooms/#{room_id}/report/#{offending_event_id}", %{
        "reason" => "spam",
        "score" => -100
      })
    ...
    unban_conn =
      authed(alice.token)
      |> jp("/_matrix/client/v3/rooms/#{room_id}/unban", %{"user_id" => charlie.user_id})
  ...
  test "a non-privileged member cannot kick or ban another member" do
    ...
    ban_conn =
      authed(bob.token)
      |> jp("/_matrix/client/v3/rooms/#{room_id}/ban", %{"user_id" => charlie.user_id})
