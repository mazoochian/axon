defmodule AxonWeb.FakeAppService do
  @moduledoc """
  A minimal in-process Application Service standing in for a real bridge in
  tests — the same pattern as `AxonFederation.FakeRemoteMatrixServer` (a
  `Plug.Router` on a real loopback Bandit port, with a canned-response
  escape hatch and a request log), for the AS-facing endpoints axon calls
  out to: `PUT .../transactions/:txnId` and the
  `GET .../thirdparty/{protocol,user,location}` family.

  Existing AS tests (`app_service_manager_test.exs`) hand-roll a one-off
  `ReceiverPlug` that only records bodies; that's enough to assert "a
  transaction arrived", but not to *answer* a query with a specific body,
  which is the whole point of the third-party lookup path. New AS tests
  should use this instead.

  Usage:

      port = 19_800
      start_supervised!({AxonWeb.FakeAppService, port: port})
      AxonWeb.FakeAppService.transaction_response(port, 200)
      registration = %{"id" => "b", "url" => "http://127.0.0.1:\#{port}", "hs_token" => "t", ...}
  """

  use Plug.Router

  plug(Plug.Parsers, parsers: [:json], pass: ["*/*"], json_decoder: Jason)
  plug(:match)
  plug(:dispatch)

  def child_spec(opts) do
    port = Keyword.fetch!(opts, :port)

    %{id: {__MODULE__, port}, start: {__MODULE__, :start_link, [opts]}, type: :supervisor}
  end

  def start_link(opts) do
    port = Keyword.fetch!(opts, :port)

    Supervisor.start_link(
      [
        %{
          id: agent_name(port),
          start:
            {Agent, :start_link,
             [fn -> %{overrides: %{}, requests: []} end, [name: agent_name(port)]]}
        },
        %{
          id: {:bandit, port},
          start: {Bandit, :start_link, [[plug: __MODULE__, ip: {127, 0, 0, 1}, port: port]]}
        }
      ],
      strategy: :one_for_all,
      name: :"#{inspect(__MODULE__)}.Supervisor#{port}"
    )
  end

  defp agent_name(port), do: :"axon_web_fake_appservice_#{port}"
  defp state(port), do: Agent.get(agent_name(port), & &1)
  defp update_state(port, fun), do: Agent.update(agent_name(port), fun)

  @doc "All requests received so far: `%{method, path, query, headers, body}` maps, oldest first."
  def requests(port), do: Enum.reverse(state(port).requests)

  def clear_requests(port), do: update_state(port, fn s -> %{s | requests: []} end)

  @doc "Escape hatch: `path_matcher` is an exact path string or a `Regex`."
  def put_response(port, {method, path_matcher}, status, body) do
    method = String.upcase(to_string(method))
    update_state(port, fn s -> put_in(s.overrides[{method, path_matcher}], {status, body}) end)
  end

  @doc "Convenience: canned status for `PUT /_matrix/app/v1/transactions/:txnId` (any txn_id)."
  def transaction_response(port, status) do
    put_response(port, {"PUT", ~r{^/_matrix/app/v1/transactions/}}, status, %{})
  end

  @doc "Convenience: canned status/body for `GET /_matrix/app/v1/thirdparty/protocol/:protocol`."
  def thirdparty_protocol_response(port, protocol, status, body) do
    put_response(port, {"GET", "/_matrix/app/v1/thirdparty/protocol/#{protocol}"}, status, body)
  end

  @doc """
  Convenience: canned status/body for `GET /_matrix/app/v1/thirdparty/user[/:protocol]`.
  The query string, if any, isn't matched — pass `protocol: nil` for the
  reverse-lookup path with no protocol suffix.
  """
  def thirdparty_user_response(port, protocol, status, body) do
    put_response(port, {"GET", thirdparty_path("user", protocol)}, status, body)
  end

  @doc "Same as `thirdparty_user_response/4` but for `.../thirdparty/location[/:protocol]`."
  def thirdparty_location_response(port, protocol, status, body) do
    put_response(port, {"GET", thirdparty_path("location", protocol)}, status, body)
  end

  defp thirdparty_path(kind, nil), do: "/_matrix/app/v1/thirdparty/#{kind}"
  defp thirdparty_path(kind, protocol), do: "/_matrix/app/v1/thirdparty/#{kind}/#{protocol}"

  match _ do
    log_request(conn)

    case find_override(conn) do
      {status, body} -> send_json(conn, status, body)
      nil -> send_json(conn, 200, %{})
    end
  end

  defp find_override(conn) do
    Enum.find_value(state(conn.port).overrides, fn
      {{method, %Regex{} = re}, resp} ->
        if conn.method == method and Regex.match?(re, conn.request_path), do: resp

      {{method, path}, resp} when is_binary(path) ->
        if conn.method == method and conn.request_path == path, do: resp
    end)
  end

  defp log_request(conn) do
    entry = %{
      method: conn.method,
      path: conn.request_path,
      query: URI.decode_query(conn.query_string || ""),
      headers: conn.req_headers,
      body: conn.body_params
    }

    update_state(conn.port, fn s -> %{s | requests: [entry | s.requests]} end)
  end

  defp send_json(conn, status, body) do
    conn
    |> Plug.Conn.put_resp_content_type("application/json")
    |> Plug.Conn.send_resp(status, Jason.encode!(body))
  end
end
